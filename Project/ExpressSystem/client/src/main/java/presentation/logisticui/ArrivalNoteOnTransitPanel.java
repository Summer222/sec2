...
public class ArrivalNoteOnTransitPanel extends JPanel {
	...
	public ArrivalNoteOnTransitPanel() {
	    ...
	    btnNewButton.setBounds(987, 222, 117, 30);
		add(btnNewButton);	
        ...
        label.setBounds(869, 297, 61, 16);
		add(label);
												
		
		barcode = new JTextField();
		barcode.setBounds(970, 290, 134, 30);
		add(barcode);
		barcode.setColumns(10);
		
		JLabel departture = new JLabel("出发地");
		departture.setBounds(869, 133, 61, 16);
        add(departture);
															
        JLabel label_2 = new JLabel("中转单编号");
        label_2.setBounds(869, 178, 74, 16);
		add(label_2);
		...
		departure1.setBounds(970, 126, 134, 30);
		add(departure1);
	    departure1.setColumns(10);													
																		
	    trannum1 = new JTextField();
	    trannum1.setBounds(970, 171, 134, 30);
		add(trannum1);
		trannum1.setColumns(10);
			
			JButton button = new JButton("添加");
			button.setBounds(987, 390, 117, 30);
			add(button);
			...
			button_1.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					res = service.submitCenterArrivalDoc(vo);
					if(res.isPass()){
						date1.enable(false);;
						centrenum2.enable(false);;
						trannum2.enable(false);;;
						departure2.enable(false);
